...
```{r}
bwplot(price~waterfront, main="House Price vs Waterfront View", xlab="Waterfront View", data=groupcdata)
```
...
```{r}
bwplot(price~condition, main="House Price vs Home Condition", xlab=" Home Condition", data=groupcdata)
```
...
```
The plot of price and year built tells us fairly little about the data, and a correlation coefficient of 0.054 is extremely weak. Year built might very well be a variable that we end up omitting from the model.
