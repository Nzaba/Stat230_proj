...
ggmap(map) +
  ...
  labs(x="Longitude", y="Latitude")+ 
    scale_size(range = c(3, 8)) + ggtitle("Sale Price of Homes in King County") + scale_alpha_continuous(guide = FALSE)
```
...
ggmap(map, extent = "device") + geom_density2d(data = groupcdata, aes(x = long, y = lat)) + stat_density2d(data = groupcdata, aes(x = long, y = lat, fill = ..level.., alpha = ..level..), size = .01, bins = 16, geom = "polygon") + scale_fill_gradient(low = "yellow", high = "red", guide = FALSE) +scale_alpha_continuous(guide = FALSE) + ggtitle("Where Sales Are Happening in King County")
```
...
ggmap(map) +
  ...
  labs(x="Longitude", y="Latitude")+ 
    scale_color_manual(values = c("darkgreen"), guide = FALSE) + ggtitle("Distribution of Sale Price in 98038 ZIP code") + scale_alpha_continuous(guide = FALSE)
```
...
ggmap(map) +
  ...
  labs(x="Longitude", y="Latitude")+ 
    scale_color_manual(values = c("darkgreen"), guide = FALSE) + ggtitle("Distribution of Sale Price in 98103 ZIP code") + scale_alpha_continuous(guide = FALSE)
```
...
ggmap(map) +
  ...
  labs(x="Longitude", y="Latitude")+ 
    scale_color_manual(values = c("darkgreen"), guide = FALSE) + ggtitle("Distribution of Sale Price in 98052 ZIP code") + scale_alpha_continuous(guide = FALSE)
```
...
ggmap(map) +
  ...
  labs(x="Longitude", y="Latitude")+ 
    scale_color_manual(values = c("darkgreen"), guide = FALSE) + ggtitle("Distribution of Sale Price in 98115 ZIP code") + scale_alpha_continuous(guide = FALSE)
```
...
ggmap(map) +
  ...
  labs(x="Longitude", y="Latitude")+ 
    scale_color_manual(values = c("darkgreen"), guide = FALSE) + ggtitle("Distribution of Sale Price in 98117 ZIP code") + scale_alpha_continuous(guide = FALSE)
```
...
### Results after a log transformation:
```{r}
xyplot(price~log(sqft_lot), main="House Price vs. Square Footage", data=sales)
with(sales, cor(price,sqft_lot))
...
After a log transformation, it is clear that some of the conditions for the linear model still haven't been met. There is still a problem with the normality condition as can be seen on the upper and lower tail of the qq plot. Additionally, the distribution of error terms seems to vary across fitted values. We shall run the randomization test below to test the significance of the relationship between price and square footage.
...
The observed correlation is 0.036. We use the randomization test and shuffle up the square footage, obtaining
1000 correlations. The resulting distribution of correlations shows that 0.036 falls within the constructed interval after destroying the relationship between price and square footage, which suggests that the correlation between these two variables is not signifiantly greater than zero (House price is unrelated to square footage). 
...
** Should we do a pairwise comparisons to see where these differences lie? **

```{r}
#Fishers LSD
library(scmamp)
require(mosaic)
PostHocTest(aov(log(price)~zipcode,data=sales), method = "lsd")
```

** Instead of Fishers LSD, I suggest using either Tukey HSD or Bonferroni, as they are more conservative tests. **

```{r}
TukeyHSD(modl)
```

The results of Tukey's HSD indicate that significant differences in home values exist between all zipcodes except the 98117 and 98103 area codes. 
...
```{r}
modall <- lm(price~bedrooms+bathrooms+sqft_lot+log(sqft_lot)+floors+waterfront+condition+yr_built+zipcode+sqft_living, data=sales)
summary(modall)
plot(modall,which=1)
plot(modall,which=2)
vif(modall)
...
modStep <- lm(log(price)~bedrooms+bathrooms+sqft_lot+log(sqft_lot)+floors+waterfront+condition+zipcode+sqft_living, data=sales)
summary(modStep)
plot(modStep, which=1)
plot(modStep, which=2)
plot(modStep, which=3)
plot(modStep, which=4)
plot(modStep, which=5)
```
